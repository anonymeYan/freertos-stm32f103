...
extern char temp111;
 u16 charge_Addr,charge_on,BSMnum,cdzt,chargeXO,ZIresetm;

extern u32 RS485_Baudrate;                       //通讯波特率
extern u8 RS485_Parity;                              //0无校验；1奇校验；2偶校验
extern u8 RS485_Addr;                                //从机地址
...
vu32 *Modbus_OutputIO[20];                     //输出开关量寄存器指针(这里使用的是位带操作)
 ...
 u16 LTC6803Err_Flag;
 u16 LTC6803_Ini_OK;
 u16 Dcc_Voltage;                //开启均衡电压
 u16 Dcc_Value;
 u8 Noise;
 u8 Resetm,contact;
 u16 SBLVoltage,SBHVoltage;
 u8 socalarm;
...
Modbus_InputIO[1]=(vu32*)&PEin(3);//KEY1
	...
	*///保持寄存器指针指向
Modbus_HoldReg[0]=(u16*) &temp111;//总电压
